...
- `Namespace` `(string: "")` <EnterpriseAlert inline /> - Specifies the namespace of the service and checks you deregister.
  This field takes precedence over the `ns` query parameter,
  one of several [other methods to specify the namespace](#methods-to-specify-namespace).
  Additionally, the namespace may be provided within the `Service` or `Check` fields,
  but if present in multiple places, they must all be the same.
...
- `near` `(string: "")` - Specifies a node name to sort the node list in
  ...
  `?near=_agent` will use the agent's node for the sort.
...
- `node-meta` `(string: "")` **Deprecated** - Use `filter` with the `Meta` selector instead.
  ...
  will filter the results to nodes with the specified key/value pairs.
...
- `node_name` `(string: <required>)` - Specifies the name of the node for which to list services.
...
Catalog endpoints
support several methods for specifying the namespace of resource(s)
